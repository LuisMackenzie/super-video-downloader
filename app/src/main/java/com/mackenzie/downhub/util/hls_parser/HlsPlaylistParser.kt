...
object HlsPlaylistParser {
    ...
    fun parse(playlistContent: String, baseUri: String): HlsPlaylist {
        try {
            AppLogger.Companion.d("HLS Parser: Attempting to parse with ExoPlayer parser. manifest body length: ${playlistContent.length}")
            return parseWithExoPlayer(playlistContent, baseUri)
        } catch (exoPlayerError: Exception) {
            AppLogger.Companion.w("HLS Parser: ExoPlayer parser failed ('${exoPlayerError.message}'). Falling back to legacy parser.")
            try {
