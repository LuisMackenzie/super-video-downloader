package com.mackenzie.downhub.util.downloaders.youtubedl_downloader
...
import com.mackenzie.downhub.util.CookieUtils
...
class YoutubeDlDownloaderWorker(appContext: Context, workerParams: WorkerParameters) :
    ...
    private fun startDownload(
        ...
        AppLogger.Companion.d("Start download dl:  ${vFormat.formatId} $url $task")
...
//        $youtube-dl --proxy http://user:password@your_proxy.com:port url
        ...
        if (currentProxy != Proxy.Companion.noProxy()) {
    ...
    private fun monitorDownloadProcess(taskId: String, task: VideoTaskItem) {
        monitorProcessDisposable =
            Observable.interval(0, 1, TimeUnit.SECONDS).subscribeOn(Schedulers.io())
                .map { FileUtil.Companion.calculateFolderSize(tmpFile) }.onErrorReturn { -1 }
                .subscribe { folderSize ->
                    if (folderSize > 0 && folderSize != lastTmpDirSize) {
                        val downloadedTmpFolderSize =
                            FileUtil.Companion.getFileSizeReadable(folderSize.toDouble())
    ...
    private fun handleError(
        ...
        AppLogger.Companion.d("Download Error: $throwable \ntaskId: $taskId")
    ...
    private class LineInfo(
        ...
        override fun toString(): String {
            return "${FileUtil.Companion.getFileSizeReadable(progress)} / ${
                FileUtil.Companion.getFileSizeReadable(
                    total
                )
    ...
    private fun deserializeVideoFormat(taskId: String): VideoFormatEntity {
        val rawHeaders = GenericDownloader.Companion.getInstance()
