...
object DownloaderUtils {
    ...
    fun mergeHlsSegments(
        ...
        AppLogger.Companion.d("DownloaderUtils: Executing HLS merge with arguments: $arguments")
        ...
        val session = FFmpegKit.executeAsync(
            ...
            { completedSession -> // This callback runs when the command finishes
                ...
                if (!ReturnCode.isSuccess(completedSession.returnCode)) {
                    AppLogger.Companion.e("FFmpeg merge failed with return code ${completedSession.returnCode}. Log: ${completedSession.allLogsAsString}")
                }
    ...
    fun downloadKey(httpClient: OkHttpClient, uri: String, keyFile: File, headers: Headers) {
        try {
            ...
            httpClient.newCall(request).execute()
                .use { response ->
                    ...
                    AppLogger.Companion.d("HLS: Encryption key downloaded to ${keyFile.absolutePath}")
                }
        } catch (e: Exception) {
            throw IOException(
    ...
    private fun createConcatenatedFmp4File(
        ...
        try {
            concatenatedFile.outputStream().use { output ->
                ...
                AppLogger.Companion.d("HLS (fMP4): Reading $prefix init segment from ${initFile.absolutePath}")
                ...
                segments.forEachIndexed { index, _ ->
                    ...
                    if (segmentFile.exists()) {
                        ...
                        if (tsFile.exists()) {
                            ...
                            AppLogger.Companion.w("HLS (fMP4): Could not find $prefix segment file: ${segmentFile.name}")
                        }
                    }
                }
            }
        ...
        AppLogger.Companion.d("HLS (fMP4): All $prefix segments concatenated into ${concatenatedFile.absolutePath}")
    ...
    private fun createTsPlaylistFile(
        ...
        if (isEncrypted) {
            AppLogger.Companion.d("HLS: Encryption detected for $filePrefix. Method: ${key.method}")
        ...
        AppLogger.Companion.d("Created playlist file: ${playlistFile.name}")
        return playlistFile
    }
