...
class CustomProxyController @Inject constructor(
    ...
    private fun setCurrentProxy(proxy: Proxy) {
        ...
        if (newConfig == lastAppliedConfig) {
            AppLogger.Companion.d("Proxy config is unchanged. No action needed.")
            return
        }

        val isProxyActive = proxy != Proxy.Companion.noProxy() || isDohEnabled

        if (isProxyActive) {
            AppLogger.Companion.d("Applying local proxy settings (127.0.0.1:8888).")
            ...
            if (WebViewFeature.isFeatureSupported(WebViewFeature.PROXY_OVERRIDE)) {
                try {
                    ...
                    AppLogger.Companion.d("ERROR SETTING PROXY: $e")
                }
            }
        } else {
            AppLogger.Companion.d("Clearing all proxy settings.")
