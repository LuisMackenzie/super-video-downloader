...
class SegmentDownloader(
    ...
    suspend fun download(
        ...
        if (outputFile.exists() && outputFile.length() > 0) {
            AppLogger.Companion.d("$logPrefix: Segment $segmentIdentifier already exists. Skipping.")
        ...
        for (attempt in 1..RETRY_COUNT) {
            ...
            try {
                AppLogger.Companion.d("$logPrefix: Downloading segment $segmentIdentifier from $segmentUrl (Attempt $attempt/$RETRY_COUNT)")
                ...
                AppLogger.Companion.d("$logPrefix: Segment $segmentIdentifier downloaded successfully.")
                ...
                lastException = e
                AppLogger.Companion.w("$logPrefix: Failed to download segment $segmentIdentifier on attempt $attempt: ${e.message}")
