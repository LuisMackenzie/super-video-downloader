package com.mackenzie.downhub.util.downloaders.super_x_downloader.strategy

import com.antonkarpenko.ffmpegkit.FFmpegKit
import com.antonkarpenko.ffmpegkit.FFmpegSession
import com.antonkarpenko.ffmpegkit.ReturnCode
import com.mackenzie.downhub.util.AppLogger
import com.mackenzie.downhub.util.downloaders.custom_downloader.CustomFileDownloader
import com.mackenzie.downhub.util.downloaders.custom_downloader.DownloadListener
...
import com.mackenzie.downhub.util.downloaders.super_x_downloader.SegmentDownloader
...
class MpdDownloader(
    ...
    override suspend fun download(
        ...
        return withContext(Dispatchers.IO) {
            ...
            if (videoRep?.isDrmProtected == true || audioRep?.isDrmProtected == true) {
                AppLogger.Companion.e("MPD: Content is DRM-protected. Download is not permitted.")
            ...
            val finalOutputFile = if (primaryRep.segments.isNotEmpty()) {
                AppLogger.Companion.d("MPD: Detected manifest with segments. Starting segment download.")
                downloadBySegments(
                    task,
                    headers,
                    downloadDir,
                ...
                AppLogger.Companion.d("MPD: Detected manifest with Base URLs. Starting file download.")
                downloadByBaseUrl(
                    task,
                    headers,
                    downloadDir,
    ...
    private suspend fun downloadBySegments(
        ...
        videoRep?.initializationUrl?.let { url ->
            val job = launch(dispatcher) {
                AppLogger.Companion.d("MPD: Downloading video init segment from $url")
        ...
        audioRep?.initializationUrl?.let { url ->
            val job = launch(dispatcher) {
                AppLogger.Companion.d("MPD: Downloading audio init segment from $url")
                segmentDownloader.download(
                    url,
        ...
        AppLogger.Companion.d("MPD (Segments): All segments downloaded. Merging...")
    ...
    private suspend fun downloadByBaseUrl(
        ...
        AppLogger.Companion.d("MPD (BaseURL): All stream downloads complete. Merging...")
        val totalDurationSeconds = videoRep?.segments?.sumOf { it.durationSeconds }
            ?: audioRep?.segments?.sumOf { it.durationSeconds } ?: 0.0
    ...
    private suspend fun downloadFileWithCustomDownloader(
        ...
        continuation.invokeOnCancellation {
            CustomFileDownloader.Companion.pause(outputFile)
        }
    ...
    private fun mergeMpdSegments(
        ...
        AppLogger.Companion.d("MPD: Starting manual concat merge. ${videoSegments?.size ?: 0} video, ${audioSegments?.size ?: 0} audio.")
        ...
        AppLogger.Companion.d("MPD: Concatenation finished. Starting FFmpeg merge.")
    ...
    private fun mergeBaseUrlStreams(
        ...
        AppLogger.Companion.d("FFmpeg: Executing MPD BaseURL merge with command: $commandString")
        ...
        val session = FFmpegKit.executeAsync(commandString, { completedSession ->
            ...
            if (!ReturnCode.isSuccess(completedSession.returnCode)) {
                AppLogger.Companion.e("FFmpeg merge failed. Log: ${completedSession.allLogsAsString}")
            }
            latch.countDown()
        }, { log ->
            AppLogger.Companion.d("FFmpeg: ${log.message}")
        }, { statistics ->
    ...
    private fun manualConcat(
        ...
        if (filesToConcat.isEmpty()) {
            AppLogger.Companion.w("ManualConcat: No files to concatenate for ${outputFile.name}")
            return
        }
        AppLogger.Companion.d("ManualConcat: Starting for ${outputFile.name}. Concatenating ${filesToConcat.size} files.")
        ...
        outputFile.outputStream().use { output ->
            ...
            filesToConcat.forEach { file ->
                if (file.exists()) {
                    ...
                    AppLogger.Companion.w("ManualConcat: File not found, skipping: ${file.name}")
                }
            }
        ...
        AppLogger.Companion.d("ManualConcat: Finished. Output size: ${outputFile.length()} bytes.")
    }
}
