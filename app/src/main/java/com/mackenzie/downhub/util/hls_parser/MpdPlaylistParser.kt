package com.mackenzie.downhub.util.hls_parser
...
object MpdPlaylistParser {
    ...
    private fun translateRepresentation(
        ...
        if (index == null) {
            AppLogger.Companion.d("MPD Parser: No segment index found for representation ${exoRep.format.id}.")
        } else {
            if (isLive) {
                // Case 1: LIVE stream logic (as determined by the top-level manifest)
                AppLogger.Companion.d("MPD Parser: Detected LIVE index for ${exoRep.format.id}.")
                try {
                    val liveEdgeUs =
                    ...
                    if (segmentNum > 0) {
                        ...
                        AppLogger.Companion.d("MPD Parser (Live): Calculated live edge segment #$segmentNum for ${exoRep.format.id}")
                    } else {
                        AppLogger.Companion.w("MPD Parser (Live): Calculated a non-positive segment number ($segmentNum), skipping.")
                    }
                } catch (e: Exception) {
                    AppLogger.Companion.e("MPD Parser (Live): Failed to calculate live edge segment ${e.message}")
                    e.printStackTrace()
                }
            } else {
                // Case 2: VOD (Static) stream logic
                AppLogger.Companion.d("MPD Parser: Detected VOD index for ${exoRep.format.id}.")
                if (periodDuration != null) {
                    ...
                    if (segmentCount > 0) {
                        ...
                        AppLogger.Companion.d("MPD Parser (VOD): Found $segmentCount segments for ${exoRep.format.id}.")
                    } else {
                        AppLogger.Companion.d("MPD Parser (VOD): No segments found for ${exoRep.format.id} (count: $segmentCount).")
                    }
                }
            }
        }
        ...
        if (drmInitData != null) {
            ...
            for (i in 0 until drmInitData.schemeDataCount) {
                ...
                AppLogger.Companion.d("MPD Parser: Found DRM scheme ${schemeData.uuid} for rep ${exoRep.format.id}")
            }
        }
