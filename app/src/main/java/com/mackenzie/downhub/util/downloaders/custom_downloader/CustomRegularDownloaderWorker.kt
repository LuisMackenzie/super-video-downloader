package com.mackenzie.downhub.util.downloaders.custom_downloader
...
import com.mackenzie.downhub.util.FfmpegProcessor
...
class CustomRegularDownloaderWorker(appContext: Context, workerParams: WorkerParameters) :
    ...
    override fun finishWork(item: VideoTaskItem?) {
        AppLogger.Companion.d("FINISHING... ${item?.filePath} $item")

        if (getDone()) {
            // Already resumed or finishing
            AppLogger.Companion.w("finishWork called but worker is already done. Ignoring.")
            return
        }

        val taskId = item?.mId ?: run {
            AppLogger.Companion.d("SMTH WRONG, taskId is NULL in finishWork $item")
            if (!getDone()) {
                setDone()
        ...
        try {
            ...
            AppLogger.Companion.e("FINISHING UNEXPECTED ERROR $item ${e.message}")
            e.printStackTrace()
            if (!getDone()) {
                setDone()
                try {
    ...
    private fun handleSuccessfulDownload(item: VideoTaskItem, srcPath: File) {
        if (outputFileName == null) {
            AppLogger.Companion.d("Output file name is NULL")
            return
        }

        AppLogger.Companion.d("handleSuccessfulDownload started for item: ${item.mId}")
        ...
        val sourcePath: File
        if (!srcPath.exists()) {
            AppLogger.Companion.w("Source file not exists in handleSuccessfulDownload: $srcPath")
            sourcePath = File(
                "${
                    File(
                        srcPath.parentFile,
            ...
            AppLogger.Companion.w("Try preprocessed name ${sourcePath.exists()} $sourcePath")
        ...
        try {
            ...
            if ((isProcessFfmpeg || isFlv) && !isPreprocessed) {
                ...
                AppLogger.Companion.d("START FFMPEG PROCESSING... $sourcePath")

                // Determine if it's an FLV stream
                AppLogger.Companion.d("IS FLV: $isFlv")
                // The FfmpegProcessor call is now correctly blocking this thread
                processedUri = FfmpegProcessor.Companion.getInstance()
                ...
                AppLogger.Companion.d("END FFMPEG PROCESSING... $processedUri")
            ...
            AppLogger.Companion.d("START MOVING... $finalSource -> $target")
            fileMovedSuccess =
                fileUtil.moveMedia(applicationContext, finalSource, File(target).toUri())
            AppLogger.Companion.d("END MOVING... fileMovedSuccess: $fileMovedSuccess")
            ...
            if (targetFl.exists()) {
                ...
                AppLogger.Companion.d("Target file exists $targetFl  marking success ${item.mId}")
                finishWork(item.also { it.taskState = VideoTaskState.SUCCESS })
            } else {
                AppLogger.Companion.e("Error during post-processing ${e.printStackTrace()}")
    ...
    private fun startDownload(taskItem: VideoTaskItem, headers: Map<String, String>) {
        AppLogger.Companion.d("Start download regular: $taskItem headers: $headers")
    ...
    private fun createDownloadListener(taskItem: VideoTaskItem, taskId: String): DownloadListener {
        return object : DownloadListener {
            override fun onSuccess() {
                AppLogger.Companion.d("Download Success, proceeding to post-processing for $outputFileName")
            ...
            override fun onFailure(e: Throwable) {
                ...
                AppLogger.Companion.e("${e.message} Download Failed for $outputFileName")
                e.printStackTrace()
                ...
                if (taskState == VideoTaskState.SUCCESS || (taskState == VideoTaskState.ERROR && taskItem.isLive)) {
                    ...
                    AppLogger.Companion.d("HANDLING TASK SUCCESS IN FAILURE $item")
                    ...
                    AppLogger.Companion.d("HANDLING TASK PAUSE IN FAILURE $taskItem")
                    finishWork(item)
                } else {
                    val item = taskItem.also {
                        it.taskState = taskState
                    ...
                    AppLogger.Companion.d("HANDLING TASK EROR IN FAILURE $item")
                    finishWork(item)
                }
            }
...
//        })
    ...
    private fun stopAndSave(task: VideoTaskItem) {
        ...
        AppLogger.Companion.d("STOPPING AND SAVING>>>>  $tmpFile")
