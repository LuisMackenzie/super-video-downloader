...
import com.mackenzie.downhub.util.downloaders.super_x_downloader.DownloaderUtils
        ...
        return withContext(Dispatchers.IO) {
            ...
            try {
                if (!mergeOnly) {
                    ...
                    AppLogger.Companion.d("HLS (Live): Starting download loop for task ${task.mId}")
                    task.setIsLive(true)

                    while (!controller.isInterrupted()) {
                        AppLogger.Companion.d("HLS (Live): Fetching latest playlist for task ${task.mId}...")
                        ...
                        if (newVideoSegments.isNotEmpty() || newAudioSegments.isNotEmpty()) {
                            AppLogger.Companion.d("HLS (Live): Found ${newVideoSegments.size} new video and ${newAudioSegments.size} new audio segments.")
                            val newDuration =
                            ...
                            AppLogger.Companion.d("HLS (Live): No new segments found.")
                        }
                        ...
                        if (isVideoFinished && isAudioFinished) {
                            AppLogger.Companion.d("HLS (Live): Stream finished naturally. Proceeding to merge.")
                            break
                        }

                        val waitTime = (targetDuration / 2 * 1000).toLong()
                        AppLogger.Companion.d("HLS (Live): Waiting for up to ${waitTime / 1000.0} seconds...")
                        interruptibleDelay(waitTime, controller)
                    }
                } else {
                    AppLogger.Companion.d("HLS (Live): Starting in MERGE-ONLY mode.")
                ...
                downloadException = e
                AppLogger.Companion.w("HLS (Live): Exception caught during download loop: ${e.message}. Attempting to save partial file.")
                e.printStackTrace()
            } finally {
                AppLogger.Companion.d("HLS (Live): Entering 'finally' block to attempt merge.")
                ...
                (allVideoSegments.firstOrNull() as? HlsPlaylistParser.UrlMediaSegment)?.encryptionKey?.let { key ->
                    ...
                    if (!keyFile.exists() || keyFile.length() == 0L) {
                        AppLogger.Companion.d("HLS: Downloading video encryption key from ${key.uri}")
                ...
                (allAudioSegments.firstOrNull() as? HlsPlaylistParser.UrlMediaSegment)?.encryptionKey?.let { key ->
                    ...
                    if (!keyFile.exists() || keyFile.length() == 0L) {
                        AppLogger.Companion.d("HLS: Downloading audio encryption key from ${key.uri}")
                        DownloaderUtils.downloadKey(httpClient, key.uri, keyFile, headers.toHeaders())
                    }
                }

                AppLogger.Companion.d("HLS (Live): Proceeding to merge ${allVideoSegments.size} video and ${allAudioSegments.size} audio segments.")
                onMergeProgress(
                ...
                if (downloadException != null) {
                    AppLogger.Companion.w("HLS (Live): Download was interrupted, but merge was successful. Returning partial file.")
    ...
    private suspend fun interruptibleDelay(
        ...
        while (System.currentTimeMillis() < endTime) {
            if (controller.isInterrupted()) {
                AppLogger.Companion.d("HLS (Live): Action detected during wait. Breaking delay.")
                break
            }
