...
class ProxyService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.action == null) {
            ...
            when (intent.action) {
                ...
                ACTION_STOP -> {
                    AppLogger.Companion.i("Stopping proxy service via explicit stop action.")
                    stopSelf()
                }
            }
        }
    ...
    private fun handleSystemStartOrRestart() {
        ...
        proxyJob = serviceScope.launch {
            ...
            if (!useProxy && !useDns) {
                AppLogger.Companion.i("Proxy and DNS are off, stopping restarted service.")
                stopSelf()
                return@launch
            }

            startForegroundWithCorrectType()
            AppLogger.Companion.i("ProxyService (re)started by system. Initializing from SharedPreferences.")

            val proxyHops = if (useProxy) {
                sharedPrefHelper.getUserProxyChain().filter { it != Proxy.Companion.noProxy() }.map { proxy ->
                    ProxyHop(
            ...
            if (success) {
                AppLogger.Companion.i("Proxy successfully restarted from background.")
            } else {
                AppLogger.Companion.e("Failed to restart proxy from background.")
            }
        }
    }
    ...
    private fun handleStartOrUpdate(intent: Intent) {
        ...
        proxyJob = serviceScope.launch {
            val localCreds = sharedPrefHelper.getGeneratedCreds()
            AppLogger.Companion.i("Starting or updating proxy chain via Intent.")
            ...
            if (success) {
                AppLogger.Companion.i("Local Proxy process started/updated successfully.")
            } else {
                AppLogger.Companion.e("Failed to start/update Local Proxy process.")
            }
        }
    }
    ...
    override fun onDestroy() {
        AppLogger.Companion.i("Local Proxy Stopping....")
        ProxyManager.stopLocalProxy()
        AppLogger.Companion.i("Local Proxy Stooped.")
        proxyJob?.cancel()
        super.onDestroy()
        isRunning = false
        AppLogger.Companion.i("ProxyService destroyed.")
    }
