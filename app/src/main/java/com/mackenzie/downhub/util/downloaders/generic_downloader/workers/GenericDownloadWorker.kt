package com.mackenzie.downhub.util.downloaders.generic_downloader.workers
...
abstract class GenericDownloadWorker(appContext: Context, workerParams: WorkerParameters) :
    ...
    private val fileDir: String = File(
        applicationContext.filesDir.absolutePath, FileUtil.Companion.FOLDER_NAME
    ).absolutePath
    ...
    override suspend fun doWork(): Result {
        return suspendCoroutine { continuation ->
            ...
            try {
                if (!ProxyService.Companion.isRunning) {
                    AppLogger.Companion.d("ProxyService is not running.")
                } else {
                    AppLogger.Companion.d("ProxyService is running.")
                }
                ...
                AppLogger.Companion.e("Invalid input: $e")
                continuation.resume(Result.failure())
            } catch (e: IOException) {
                AppLogger.Companion.e("Download error:- $e")
                continuation.resume(Result.failure())
            } catch (e: Exception) {
                AppLogger.Companion.e("Unexpected error: $e")
                continuation.resume(Result.failure())
            }
        }.also {
            afterDone()
    ...
    private fun loadHeaders(taskId: String): Map<String, String> {
        val inpHeaders =
            GenericDownloader.Companion.getInstance()
        ...
        return inpHeaders?.let {
            try {
                val decodedHeaders =
                    String(
                        Base64.decode(
                            GenericDownloader.Companion.getInstance().decompressString(it),
                            Base64.DEFAULT
                        )
                    )
