...
import com.mackenzie.downhub.data.local.GeneratedProxyCreds
...
class SharedPrefHelper @Inject constructor(
    ...
    fun getUserProxyChain(): Array<Proxy> {
        ...
        return arrayOf(Proxy.Companion.noProxy())
    }
    ...
    fun getGeneratedCreds(): GeneratedProxyCreds {
        ...
        return if (creds != null) {
            val saved = GeneratedProxyCreds.Companion.fromJson(creds)

            val hasSpecialChar = saved.localPassword.any { !it.isLetterOrDigit() }
            if (hasSpecialChar) {
                val newCreds = GeneratedProxyCreds.Companion.generateProxyCredentials()
                setGeneratedCreds(newCreds)
                return newCreds
            }
            saved
        } else {
            val initialCreds = GeneratedProxyCreds.Companion.generateProxyCredentials()
