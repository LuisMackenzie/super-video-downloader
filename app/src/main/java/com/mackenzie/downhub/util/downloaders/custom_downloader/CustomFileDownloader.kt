...
class CustomFileDownloader(
    ...
    fun download() {
        ...
        if (!isUrlSupportBytesRangeHeader || (this.threadCount == 1 && isForceStreamDownloadMode)) {
            val result = executorService.submit {
                AppLogger.Companion.d("Range header not supported, falling back to single-threaded download.")
        ...
        if (allPartsSucceed && !isPaused) {
            ...
            AppLogger.Companion.d("CHUNKS STOPPED AND SAVE")
            this.onFailure(Error(STOPPED_AND_SAVE_ACTION))
        } else if (isPaused) {
            AppLogger.Companion.d("CHUNKS PAUSED")
            this.onFailure(Error(PAUSE_ACTION))
        } else if (isCanceled) {
            AppLogger.Companion.d("CHUNKS CANCELED")
            this.onFailure(Error(CANCELED_ACTION))
        } else {
            AppLogger.Companion.d("CHUNKS ERROR")
    ...
    private fun downloadRegularStream(fileChannel: FileChannel) {
        ...
        if (contentLength == -1L) {
            AppLogger.Companion.w("Content length is unknown for single-threaded download.")
    ...
    private fun onSuccess() {
        ...
        AppLogger.Companion.d("DOWNLOAD SUCCESS: $file")
    ...
    private fun onFailure(e: Throwable) {
        ...
        AppLogger.Companion.e("Task Download Failed $e")
    ...
    private fun onChunkFailure(e: Throwable, index: Chunk) {
        AppLogger.Companion.e("Chunk $index Download Failed ${e.printStackTrace()}")
