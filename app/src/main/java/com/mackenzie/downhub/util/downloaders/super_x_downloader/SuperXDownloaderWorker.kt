package com.mackenzie.downhub.util.downloaders.super_x_downloader
...
import com.mackenzie.downhub.util.downloaders.generic_downloader.GenericDownloader
import com.mackenzie.downhub.util.downloaders.generic_downloader.models.VideoTaskItem
import com.mackenzie.downhub.util.downloaders.generic_downloader.models.VideoTaskState
import com.mackenzie.downhub.util.downloaders.generic_downloader.workers.GenericDownloadWorkerWrapper
import com.mackenzie.downhub.util.downloaders.generic_downloader.workers.Progress
import com.mackenzie.downhub.util.downloaders.super_x_downloader.control.FileBasedDownloadController
import com.mackenzie.downhub.util.downloaders.super_x_downloader.strategy.HlsDownloader
import com.mackenzie.downhub.util.downloaders.super_x_downloader.strategy.HlsLiveDownloader
import com.mackenzie.downhub.util.downloaders.super_x_downloader.strategy.MpdDownloader
import com.mackenzie.downhub.util.downloaders.super_x_downloader.strategy.MpdLiveDownloader
import com.mackenzie.downhub.util.hls_parser.HlsPlaylistParser
import com.mackenzie.downhub.util.hls_parser.MpdPlaylistParser
...
class SuperXDownloaderWorker(appContext: Context, workerParams: WorkerParameters) :
        ...
        try {
            when (action) {
                ...
                GenericDownloader.DownloaderActions.CANCEL -> {
                    AppLogger.Companion.d("HLS: Cancel action received for task $taskId. Creating flag file.")
                    val isWorkerRunning =
                        GenericDownloader.Companion.isWorkScheduled(applicationContext, taskId)
                    if (isWorkerRunning) {
                ...
                GenericDownloader.DownloaderActions.PAUSE -> {
                    AppLogger.Companion.d("HLS: Pause action received for task $taskId. Creating flag file.")
                    controller.requestPause()
                    getContinuation().resume(Result.success())
                }

                GenericDownloader.DownloaderActions.STOP_SAVE_ACTION -> {
                    AppLogger.Companion.d("HLS (Live): Stop and Save action received for task $taskId.")
                    val isWorkerRunning =
                        GenericDownloader.Companion.isWorkScheduled(applicationContext, taskId)

                    if (isWorkerRunning) {
                        AppLogger.Companion.d("HLS (Live): Worker is active. Signaling it to stop and save.")
                        controller.requestStopAndSave()
                        getContinuation().resume(Result.success()) // Acknowledge the request
                    } else {
                        AppLogger.Companion.d("HLS (Live): No active worker found. Starting a new merge-only job.")
            ...
            AppLogger.Companion.e("SuperX: Failed to create state flag file. $e")
        }
    }
    ...
    private fun startDownload(task: VideoTaskItem, headersRaw: Map<String, String>) {
        ...
        AppLogger.Companion.d("Custom M3U8/MPD: Starting download for initial taskId: $taskId with duration: $taskDuration\ntask: $task")
        ...
        if (isHlsPlaylist()) {
            AppLogger.Companion.d("HLS Playlist detected. Starting advanced parsing flow.")
            if (isLive) {
            ...
            AppLogger.Companion.d("MPD Manifest detected. Starting advanced parsing flow.")
            if (isLive) {
    ...
    private fun startLiveHlsDownloadLoop(
        ...
        AppLogger.Companion.d("HLS (Live): Delegating download for task $taskId to HlsLiveDownloader strategy.")
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // 3. Handle success
                AppLogger.Companion.d("HLS (Live): Strategy download completed successfully.")
                ...
                when {
                    controller.isCancelRequested() -> {
                        AppLogger.Companion.d("HLS (Live): Task $taskId was canceled.")
                        finishWork(task.apply { taskState = VideoTaskState.CANCELED })
                        return@launch
                    }

                    controller.isPauseRequested() || e is CancellationException -> {
                        AppLogger.Companion.d("HLS (Live): Task $taskId is pausing gracefully.")
                        ...
                        AppLogger.Companion.e("HLS (Live): Download failed for task $taskId: ${e.message}")
                        e.printStackTrace()
                        finishWork(task.also {
    ...
    private fun startMpdDownload(task: VideoTaskItem, headers: Map<String, String>) {
        AppLogger.Companion.d("MPD: Delegating download for task $taskId to MpdDownloader strategy.")
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // 3. Handle success
                AppLogger.Companion.d("MPD: Strategy download completed successfully.")
                val completedTask = task.also {
                    it.taskState = VideoTaskState.SUCCESS
                    it.filePath = finalOutputFile.absolutePath
                    it.totalSize = finalOutputFile.length()
                ...
                when {
                    controller.isCancelRequested() -> {
                        AppLogger.Companion.d("MPD: Task $taskId was canceled by user.")
                        finishWork(task.also { it.taskState = VideoTaskState.CANCELED })
                        return@launch
                    }

                    controller.isPauseRequested() || e is CancellationException -> {
                        AppLogger.Companion.d("MPD: Task $taskId is pausing gracefully.")
                        finishWork(task.also {
                            it.taskState = VideoTaskState.PAUSE
                            it.errorMessage = "Paused"
                        })
                        ...
                        AppLogger.Companion.e("MPD: Download failed for task $taskId: ${e.message}")
                        e.printStackTrace()
                        finishWork(task.also {
                            it.taskState = VideoTaskState.ERROR
    ...
    private fun startMpdLiveDownload(task: VideoTaskItem, headers: Map<String, String>) {
        AppLogger.Companion.d("MPD (Live): Delegating download for task $taskId to MpdLiveDownloader strategy.")
        val mpdTmpDir = fileUtil.tmpDir.resolve(taskId)
        val controller = FileBasedDownloadController(mpdTmpDir)
        controller.start()
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // 3. Handle success
                AppLogger.Companion.d("MPD (Live): Strategy download completed successfully.")
                val completedTask = task.also {
                    it.taskState = VideoTaskState.SUCCESS
                    it.filePath = finalOutputFile.absolutePath
                    it.totalSize = finalOutputFile.length()
                ...
                when {
                    e is CancellationException -> {
                        AppLogger.Companion.d("MPD (Live): Task $taskId was stopped by user. Checking for merged file.")
                        ...
                        if (finalFile.exists() && finalFile.length() > 500) { // Check for a reasonable file size
                            AppLogger.Companion.d("MPD (Live): Merge after stop was successful.")
                            ...
                            AppLogger.Companion.d("MPD (Live): Task $taskId was canceled before any segments could be merged.")
                            finishWork(task.apply { taskState = VideoTaskState.CANCELED })
                        }
                        return@launch
                    }
                        ...
                        AppLogger.Companion.e("MPD (Live): Download failed for task $taskId: ${e.message}")
                        e.printStackTrace()
                        finishWork(task.also {
                            it.taskState = VideoTaskState.ERROR
    ...
    private fun getMpdRepresentations(
        ...
        val selectedFormatId = inputData.getString(GenericDownloader.Constants.SELECTED_FORMAT_ID)
        AppLogger.Companion.d("MPD: User selected format ID: $selectedFormatId")
        ...
        when {
            selectedVideoRep != null -> {
                ...
                AppLogger.Companion.d("MPD: Matched VIDEO format. Will use ${finalVideoRep.height}p and find best audio.")
            }
            ...
            selectedAudioRep != null -> {
                ...
                AppLogger.Companion.d("MPD: Matched AUDIO format. Preparing audio-only download.")
            }
        ...
        AppLogger.Companion.d("MPD: Final Selected Video: ${finalVideoRep?.let { "${it.width}x${it.height}" } ?: "None"}")
        AppLogger.Companion.d("MPD: Final Selected Audio: ${finalAudioRep?.codecs ?: "None"}")

        return Pair(finalVideoRep, finalAudioRep)
    }

    private fun startHlsDownload(task: VideoTaskItem, headers: Map<String, String>) {
        AppLogger.Companion.d("HLS: Delegating download for task $taskId to HlsDownloader strategy.")
        val hlsTmpDir = fileUtil.tmpDir.resolve(taskId)
        val controller = FileBasedDownloadController(hlsTmpDir)
        controller.start()
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // 3. Handle success.
                AppLogger.Companion.d("HLS: Strategy download completed successfully.")
                val completedTask = task.also {
                    it.taskState = VideoTaskState.SUCCESS
                    it.filePath = finalOutputFile.absolutePath
                    it.totalSize = finalOutputFile.length()
                ...
                when {
                    controller.isCancelRequested() -> {
                        AppLogger.Companion.d("HLS: Task $taskId was canceled by user.")
                        finishWork(task.apply { taskState = VideoTaskState.CANCELED })
                        return@launch
                    }

                    controller.isPauseRequested() || e is CancellationException -> {
                        AppLogger.Companion.d("HLS: Task $taskId is pausing gracefully.")
                        finishWork(task.also {
                            it.taskState = VideoTaskState.PAUSE
                            it.errorMessage = "Paused"
                        })
                    ...
                    // Handle other errors.
                        ...
                        AppLogger.Companion.e("HLS: Download failed for task $taskId: ${e.message}")
                        e.printStackTrace()
                        finishWork(task.also {
                            it.taskState = VideoTaskState.ERROR
    ...
    private fun getMediaSegments(
        ...
        // 1. Get the selected format ID from the worker's input data.
        val selectedFormatId = inputData.getString(GenericDownloader.Constants.SELECTED_FORMAT_ID)
        AppLogger.Companion.d("HLS: User selected format ID: $selectedFormatId")
        ...
        when {
            selectedVideoVariant != null -> {
                ...
                AppLogger.Companion.d("HLS: Matched VIDEO format. Selecting variant with URL ${selectedVideoVariant.url}.")
                val videoMediaPlaylist = fetchAndParse(selectedVideoVariant.url)
                if (videoMediaPlaylist is HlsPlaylistParser.MediaPlaylist) {
                ...
                if (audioGroupId != null) {
                    ...
                    if (audioRendition?.url != null) {
                        AppLogger.Companion.d("HLS: Found separate audio rendition at URL: ${audioRendition.url}")
                        val audioMediaPlaylist = fetchAndParse(audioRendition.url)
                        if (audioMediaPlaylist is HlsPlaylistParser.MediaPlaylist) {
            ...
            selectedAudioRendition != null -> {
                ...
                AppLogger.Companion.d("HLS: Matched AUDIO format. Preparing audio-only download from URL: ${selectedAudioRendition.url}")
    ...
    override fun finishWork(item: VideoTaskItem?) {
        ...
        val taskId = item?.mId ?: run {
            AppLogger.Companion.e("SuperX: Cannot finish work, taskId is NULL")
            getContinuation().resume(Result.failure())
            return
        }
        AppLogger.Companion.d("FFmpeg: Finishing work for task $taskId with state ${item.taskState}")
        ...
        try {
            ...
            AppLogger.Companion.e("SuperX: Could not resume continuation: ${e.message}")
        }
    }
    ...
    private fun handleTaskCompletion(item: VideoTaskItem) {
        ...
        when (item.taskState) {
            VideoTaskState.CANCELED -> {
                ...
                AppLogger.Companion.d("Task cancelled by user, removing task's dir: $taskDir")
            ...
            VideoTaskState.SUCCESS -> {
                ...
                AppLogger.Companion.d("MOVING FILE $from to -> $to")
                saveProgress(
                    item.mId,
                    finalProgress,
                ...
                if (fileMoved) {
                    AppLogger.Companion.d("SuperX: File moved successfully to $targetPath")
                    ...
                    AppLogger.Companion.e("FFmpeg: Failed to move file to $targetPath")
                ...
                AppLogger.Companion.d("SuperX: Task failed with error: ${item.errorMessage}  ${item.taskState}")
                saveProgress(
                    item.mId,
                    finalProgress,
                    item.taskState,
        ...
        return headers.toMutableMap().also { fixedHeaders ->
            fixedHeaders["Cookie"]?.let {
                try {
                    ...
                    AppLogger.Companion.e("SuperX: Failed to decode Base64 Cookie: $it")
                }
            }
        }
    }
    ...
    private fun finishWorkWithFailureTaskId(task: VideoTaskItem) {
        AppLogger.Companion.d("SMTH WRONG, taskId is NULL  $task")
        try {
            getContinuation().resume(Result.failure())
        } catch (e: Throwable) {
            e.printStackTrace()
