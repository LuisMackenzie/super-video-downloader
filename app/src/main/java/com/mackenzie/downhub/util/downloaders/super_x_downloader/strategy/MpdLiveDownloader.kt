...
class MpdLiveDownloader(
    ...
    override suspend fun download(
        ...
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                AppLogger.Companion.d("MPD (Live): Starting recording loop for task ${task.mId}")
                task.setIsLive(true)
                ...
                while (currentCoroutineContext().isActive && !controller.isInterrupted()) {
                    ...
                    if (newVideoSegments.isNotEmpty() || newAudioSegments.isNotEmpty()) {
                        AppLogger.Companion.d("MPD (Live): Found ${newVideoSegments.size} new video and ${newAudioSegments.size} new audio segments.")
                        ...
                        AppLogger.Companion.d("MPD (Live): No new segments found.")
                    }

                    if (videoRep?.manifest?.type == "static" || audioRep?.manifest?.type == "static") {
                        AppLogger.Companion.d("MPD (Live): Stream type changed to 'static'. Ending recording.")
                        break
                    }
                ...
                downloadException = e
                AppLogger.Companion.w("MPD (Live): Exception in recording loop: ${e.message}. Attempting to save captured segments.")
            } finally {
                AppLogger.Companion.d("MPD (Live): Entering 'finally' block to attempt merge.")
                val capturedVideo =
                ...
                if (downloadException != null) {
                    AppLogger.Companion.w("MPD (Live): Recording was interrupted, but merge was successful. Returning partial file.")
                }
            }
    ...
    private suspend fun downloadInitSegments(
        ...
        coroutineScope {
            videoRep?.initializationUrl?.let { url ->
                launch {
                    AppLogger.Companion.d("MPD Live: Downloading video init segment.")
            ...
            audioRep?.initializationUrl?.let { url ->
                launch {
                    AppLogger.Companion.d("MPD Live: Downloading audio init segment.")
                    segmentDownloader.download(
    ...
    private fun mergeBaseUrlStreams(
        ...
        AppLogger.Companion.d("FFmpeg: Executing MPD Live merge with command: $commandString")
