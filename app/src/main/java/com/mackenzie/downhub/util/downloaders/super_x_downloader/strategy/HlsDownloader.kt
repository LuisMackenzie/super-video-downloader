...
class HlsDownloader(
    ...
    override suspend fun download(
        ...
        return withContext(Dispatchers.IO) {
            ...
            AppLogger.Companion.d("HLS: Found ${videoSegments?.size ?: 0} video and ${audioSegments?.size ?: 0} audio segments.")
            ...
            if (isVideoFmp4) {
                ...
                if (!initFile.exists() || initFile.length() == 0L) {
                    AppLogger.Companion.d("HLS (fMP4): Downloading video init segment from ${initSegment.url}")
            ...
            if (isAudioFmp4) {
                ...
                if (!initFile.exists() || initFile.length() == 0L) {
                    AppLogger.Companion.d("HLS (fMP4): Downloading audio init segment from ${initSegment.url}")
            ...
            if (initialSegmentsCompleted > 0) {
                AppLogger.Companion.d("HLS Resumed: ${alreadyDownloadedVideo.size}/${videoSegments?.size ?: 0} video and ${alreadyDownloadedAudio.size}/${audioSegments?.size ?: 0} audio segments already present.")
            ...
            AppLogger.Companion.d("HLS: All segments downloaded successfully. Starting merge.")
            ...
            AppLogger.Companion.d("HLS: Merge completed successfully.")
            finalOutputFile
        }
    }
}
