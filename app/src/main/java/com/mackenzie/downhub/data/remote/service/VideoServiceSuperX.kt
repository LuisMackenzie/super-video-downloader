...
import com.mackenzie.downhub.data.local.model.VideoInfoWrapper
import com.mackenzie.downhub.data.local.room.entity.VideFormatEntityList
import com.mackenzie.downhub.data.local.room.entity.VideoFormatEntity
...
class VideoServiceSuperX(
    ...
    override fun getVideoInfo(
        ...
        return try {
            ...
            AppLogger.Companion.d("PlaylistService Error: Failed to parse manifest. ${e.message}")
            null
        }
    }
    ...
    private fun handlePlaylistUrl(
        ...
        AppLogger.Companion.d("PlaylistService: Fetching manifest from $urlString")

        // 1. Fetch the manifest content
        val response = client.getProxyOkHttpClient().newCall(url).execute()
        val content = response.body.string()
        AppLogger.Companion.d("Manifest body: $content")
        ...
        return if (isM3u8) {
            AppLogger.Companion.d("PlaylistService: Detected HLS manifest.")
            val manifest = HlsPlaylistParser.parse(content, urlString)
            parseHlsManifest(manifest, url.headers.toMap())
        } else if (isMpd) {
            AppLogger.Companion.d("PlaylistService: Detected MPD manifest.")
            val manifest = MpdPlaylistParser.parse(content, urlString)
            parseMpdManifest(manifest, url.headers.toMap())
        } else {
            AppLogger.Companion.w("PlaylistService: URL was flagged as a playlist but extension is not .m3u8 or .mpd.")
            null
        }
    }
    ...
    private fun parseHlsManifest(
        ...
        when (manifest) {
            is HlsPlaylistParser.MasterPlaylist -> {
                ...
                if (manifest.variants.isEmpty()) {
                    AppLogger.Companion.d("HLS Parse: Master playlist has no variants.")
                    return null
                }
        ...
        if (formats.isEmpty()) {
            AppLogger.Companion.d("HLS Parse: No suitable video formats could be created.")
            return null
        }
    ...
    private fun parseMpdManifest(
        ...
        if (allVideoRepresentations.isEmpty()) {
            AppLogger.Companion.d("MPD Parse: No video representations found.")
            return null
        }
        ...
        if (formats.isEmpty()) {
            AppLogger.Companion.d("MPD Parse: No suitable video representations found.")
            return null
        }
    ...
    private fun parseIso8601Duration(duration: String?): Long {
        ...
        return try {
            ...
            AppLogger.Companion.e("Failed to parse ISO 8601 duration string: $duration \n${e.printStackTrace()}")
            0L
        }
    }
    ...
    private fun fetchFirstMediaPlaylist(
        ...
        return try {
            ...
            AppLogger.Companion.e("Failed to fetch child media playlist: $firstVariantUrl ${e.printStackTrace()}")
            null
        }
    }
}
