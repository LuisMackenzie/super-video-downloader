...
import com.mackenzie.downhub.ui.main.home.browser.DownloadButtonState
...
open class VideoDetectionTabViewModel @Inject constructor(
    ...
    override fun onStartPage(url: String, userAgentString: String) {
        if (url == lastUrl) {
            AppLogger.Companion.d("onStartPage: URL is the same. Not clearing list.")
            return
        }
        lastUrl = url
        ...
        if (url != initialUrl) {
            AppLogger.Companion.d("onStartPage: URL is not initial url. Clearing list.")
            detectedVideosList.set(mutableSetOf())
            cancelAllCheckJobs()
        } else {
            AppLogger.Companion.d("onStartPage: URL is initial url. Skipped clearing list.")
        }
            ...
            webTabModel?.getTabTextInput()?.get()?.let {
                if (it.startsWith("http")) {
                    viewModelScope.launch(executorRegular) {
                        onReloadPage(
                            it.trim(),
                            webTabModel?.userAgent?.get() ?: BrowserFragment.Companion.MOBILE_USER_AGENT
                        )
                    }
                }
            }
    ...
    open fun pushNewVideoInfoToAll(newInfo: VideoInfo) {
        ...
        if (detectedVideos.any { isVideoInfoDuplicate(it, newInfo) }) {
            AppLogger.Companion.d("SKIP DUPLICATED VIDEO INFO: $newInfo")
            return
        }

        AppLogger.Companion.d("PUSHING $newInfo to list: \n  $detectedVideos")
    ...
    override fun checkRegularVideoOrAudio(
        ...
        val disposable = io.reactivex.rxjava3.core.Observable.create<Unit> {
            ...
            AppLogger.Companion.d("Checking ERROR... $clearedUrl")
        }.subscribe()
        ...
        try {
            ...
            if (alternativeHeaders.isEmpty()) {
                ...
                try {
                    ...
                    AppLogger.Companion.d("Url parse error ${e.message}")
                }
                return builder
