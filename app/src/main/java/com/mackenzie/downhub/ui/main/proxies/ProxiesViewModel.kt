...
import com.mackenzie.downhub.util.DnsStampHelper
...
class ProxiesViewModel @Inject constructor(
    ...
    override fun start() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            realProxyCount.set(usersProxy.count { it != Proxy.Companion.noProxy() })
    ...
    fun turnOffProxy() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            proxyController.updateProxyState()
            isProxyOn.set(false)
            updateChain(listOf(Proxy.Companion.noProxy()))
        }
    }
    ...
    private fun updateChain(proxies: List<Proxy>) {
        ...
        if (!useProxy && !useDns) {
            val intent =
                Intent(ContextUtils.getApplicationContext(), ProxyService::class.java).apply {
                    action = ProxyService.Companion.ACTION_STOP
                }
        ...
        val proxyHops = if (useProxy) {
            proxies.filter { it != Proxy.Companion.noProxy() }.map { proxy ->
                ProxyHop(
        ...
        val intent = Intent(ContextUtils.getApplicationContext(), ProxyService::class.java).apply {
            action = ProxyService.Companion.ACTION_START_OR_UPDATE
            putExtra(ProxyService.Companion.EXTRA_PROXY_HOPS, ArrayList(proxyHops))
            putExtra(ProxyService.Companion.EXTRA_DNS_URL, dnsUrl)
        }
    ...
    private fun saveAndUpdateProxies(updatedList: List<Proxy>) {
        viewModelScope.launch(Dispatchers.IO) {
            val cleanedList = if (updatedList.size > 1) {
                updatedList.filter { it != Proxy.Companion.noProxy() }
            } else {
                updatedList
            }
            ...
            realProxyCount.set(cleanedList.count { it != Proxy.Companion.noProxy() })
            proxyController.updateProxyState()
