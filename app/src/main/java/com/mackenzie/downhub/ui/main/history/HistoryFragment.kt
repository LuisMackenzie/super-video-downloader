...
import com.mackenzie.downhub.ui.component.adapter.HistoryListener
...
class HistoryFragment : BaseFragment() {
    ...
    private val historyListener = object : HistoryListener {
        override fun onHistoryOpenClicked(view: View, id: String) {
            AppLogger.Companion.d("onHistoryOpenClicked: $id")
            historyModel.historyItems.get()?.find {
                it.id == id
            }.let {
                it?.let { item ->
                    parentFragmentManager.popBackStack()
                    BrowserViewModel.Companion.instance?.openPageEvent?.value =
    ...
    private val searchHistoryListener = object : HistoryListener {
        override fun onHistoryOpenClicked(view: View, id: String) {
            AppLogger.Companion.d("SEARCH: onHistoryOpenClicked  $id")
