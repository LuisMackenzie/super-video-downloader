...
import com.mackenzie.downhub.ui.main.home.browser.detectedVideos.IVideoDetector
...
class CustomWebViewClient(
    ...
    override fun shouldInterceptRequest(
        ...
        if (isCheckOnMp4 || isCheckM3u8 || isCheckOnAudio) {
            ...
            val contentType =
                VideoUtils.Companion.getContentTypeByUrl(url, requestWithCookies?.headers, okHttpProxyClient)
    ...
    private suspend fun saveUrlToHistory(url: String, favicon: Bitmap?, title: String?) {
        ...
        if (!isTitleEmpty && lastSavedTitleHistory != title && lastSavedHistoryUrl != url && url.isNotEmpty() && !url.contains(
            ...
            val outputFavicon = FaviconUtils.Companion.bitmapToBytes(favicon)
