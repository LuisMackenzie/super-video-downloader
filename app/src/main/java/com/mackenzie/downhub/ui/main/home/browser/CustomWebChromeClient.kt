...
class CustomWebChromeClient(
    ...
    override fun onPermissionRequest(request: PermissionRequest?) {
        ...
        if (isDrmRequest) {
            if (settingsViewModel.isDrmEnabled.get()) {
                AppLogger.Companion.d("DRM: Granting permission based on existing setting.")
                ...
                MaterialAlertDialogBuilder(mainActivity)
                    ...
                    .setPositiveButton(R.string.allow) { _, _ ->
                        AppLogger.Companion.d("DRM: User granted permission via dialog.")
                        request.grant(arrayOf(PermissionRequest.RESOURCE_PROTECTED_MEDIA_ID))
                        settingsViewModel.isDrmEnabled.set(true)
                    }
                    .setNegativeButton(R.string.block) { _, _ ->
                        AppLogger.Companion.d("DRM: User denied permission via dialog.")
                        request.deny()
                    }
                    .show()
                return
        ...
        AppLogger.Companion.d("Permissions: Denying non-DRM request for resources: ${request.resources.joinToString()}")
        request.deny()
    }
    ...
    override fun onCreateWindow(
        ...
        AppLogger.Companion.d("ON_CREATE_WINDOW: URL from HitTestResult: $url")

        if (!url.startsWith("http")) {
            AppLogger.Companion.d("ON_CREATE_WINDOW: Blocking ad or non-http scheme: $url")
